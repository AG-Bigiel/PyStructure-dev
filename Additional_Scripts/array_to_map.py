...
def array_to_map(ra,dec,val,nearest=False,cutoff=3.0,kernel=10.0,gridspacing=1.0):
    ...
    nra  = round(  (( ra.max()-ra.min()+xborder ) * np.cos(np.deg2rad(np.mean(dec))))  / res ,0)
    ndec = round(  (dec.max()-dec.min()+yborder )   / res ,0)
    ...
    if nearest:   
        datamap = np.array([[None for a in range(w)] for b in range(h)],dtype=np.float64)
    ...
    ###############################################
        ...
        datamap = np.array([[0.0 for a in range(w)] for b in range(h)],dtype=np.float64)
        weightmap = np.array([[0.0 for a in range(w)] for b in range(h)],dtype=np.float64)
