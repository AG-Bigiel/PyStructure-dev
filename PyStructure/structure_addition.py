...
def add_kin_to_struct(struct={}, band="", unit="", desc=""):
    """
    ; NAME:
    ;
    ; add_kin_to_struct
    ...
    ; PURPOSE:
    ;
    ; Utility for building an kinematic database.
    ; "KIN" stands for kinematc
    ;
    ; USAGE:
    ;
    ; new_struct = add_kin_to_struct(struct=old_struct, band="bandname")

    """

    tags = list(struct.keys())

    kin_name = "KIN_V_"+band.upper() # los velocities
    uc_name = "KIN_V_UC_"+band.upper() # los velocities uncertainty
    unit_name = "KIN_UNIT_"+band.upper()
    desc_name = "INT_DESC_"+band.upper()

    effwidth_name = "KIN_EFFWIDTH_"+band.upper() # velocity dispersion
    mom2_name = "KIN_MOM2_"+band.upper() # velocity dispersion
    ...
    new_struct[kin_name] = np.nan
    new_struct[uc_name] = np.nan
    new_struct[unit_name] = unit
    new_struct[desc_name]= desc
    new_struct[effwidth_name] = np.nan
